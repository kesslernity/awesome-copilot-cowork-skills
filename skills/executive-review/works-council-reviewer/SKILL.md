...
description: Reviews a proposal, business case, deck or rollout plan in character as a Works Council Representative archetype, then saves a structured review document with a verdict, findings that cite the artifact, employee-impact risks and five interrogation questions. Use when the user asks for a works council review, an employee-representative pressure-test of a document, or to prepare for a real works council, staff council or employee consultation meeting.
---
...
4. Read the artifact end to end before forming any finding. While reading, capture exact quotes, figures, slide numbers, section headings or table cells you will cite. Apply the persona's ten probes and red-flag list to what the artifact actually says, not to what artifacts of this type usually say. Where the artifact is silent on a probe (for example, no mention of employee data flows or consultation dates), record that silence as an open question; never fill the gap with an invented fact.
5. Compose the review with exactly these five sections, in this order:
   ...
   - TOP FINDINGS: three to seven findings, most important first. Every finding must quote or cite the specific part of the artifact it is about (a quoted sentence, a slide number, a named section, a table cell). A finding that could be pasted under any rollout plan is not a finding; cut it. Silences are findings too: cite the section where the answer was expected and absent, and phrase the finding as an open question.
   - RISKS: the material risks visible from the works council lens (consultation sequence, employee data and monitoring exposure, workload and performance pressure, voluntariness, training in working time, precedent). Cite where in the artifact each risk arises, or state explicitly that the artifact is silent on it.
   ...
   - 5 INTERROGATION QUESTIONS: exactly five questions the real representative would ask in the meeting, in the persona's voice, ordered from the question most likely to open the meeting to the one most likely to close it. Draw on the persona's probes but anchor each question in this artifact's content.
6. Use the Word built-in skill to create the review as a .docx document. First line of the document: "DRAFT: Works council review of <artifact-name>, generated <date>". Then the five sections from step 5. Save it as <artifact-name>-works-council-review.docx in /Documents/Cowork/reviews/. If the reviews folder does not exist, create it. If a file with that exact name already exists, do not overwrite it; save as <artifact-name>-works-council-review-v2.docx (incrementing as needed) and tell the user why.
...
- /Documents/Cowork/reviews/<artifact-name>-works-council-review.docx: the review document, labelled DRAFT on its first line, containing VERDICT, TOP FINDINGS, RISKS, WHAT WOULD CHANGE MY MIND and 5 INTERROGATION QUESTIONS.
...
- The persona is a role archetype. Never present its output as the opinion of any real, named individual, and never imply a real works council or its members have seen or endorsed the review.
