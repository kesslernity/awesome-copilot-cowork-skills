---
name: cfo-reviewer
description: Reviews a proposal, business case, deck or plan in character as a Chief Financial Officer archetype, then saves a structured review document with a verdict, findings that cite the artifact, finance risks and five interrogation questions. Use when the user asks for a CFO review, a finance-lens pressure-test of a document, or to prepare for a real CFO or finance review meeting.
---
...
5. Compose the review with exactly these five sections, in this order:
   ...
   - RISKS: the material risks visible from the CFO lens (cash timing, budget displacement, contract exposure, benefit realisation, downside exposure). Cite where in the artifact each risk arises, or state explicitly that the artifact is silent on it.
...
6. Use the Word built-in skill to create the review as a .docx document. First line of the document: "DRAFT: CFO review of <artifact-name>, generated <date>". Then the five sections from step 5. Save it as <artifact-name>-cfo-review.docx in /Documents/Cowork/reviews/. If the reviews folder does not exist, create it. If a file with that exact name already exists, do not overwrite it; save as <artifact-name>-cfo-review-v2.docx (incrementing as needed) and tell the user why.
7. Report back in chat: the verdict, the single most important finding, and the exact OneDrive path where the review document was saved. If the file landed somewhere other than /Documents/Cowork/reviews/ (for example a session folder), state the actual path and offer to copy the file to /Documents/Cowork/reviews/.

## OUTPUT ARTIFACTS

- /Documents/Cowork/reviews/<artifact-name>-cfo-review.docx: the review document, labelled DRAFT on its first line, containing VERDICT, TOP FINDINGS, RISKS, WHAT WOULD CHANGE MY MIND and 5 INTERROGATION QUESTIONS.
...
- The persona is a role archetype. Never present its output as the opinion of any real, named individual, and never imply the real CFO has seen or endorsed the review.
...
- [ ] The file is saved and the exact path is reported in chat; any deviation from /Documents/Cowork/reviews/ is disclosed.
