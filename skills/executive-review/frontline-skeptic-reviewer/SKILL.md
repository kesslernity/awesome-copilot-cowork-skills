---
name: frontline-skeptic-reviewer
description: Reviews a proposal, business case, deck or rollout plan in character as a Frontline Skeptic archetype, the experienced staff member who will have to live with the change, then saves a structured review with a verdict, findings that cite the artifact, adoption risks and five interrogation questions. Use when the user asks for a frontline review, an adoption-lens pressure-test of a plan, or asks what the people who do the work will say about it.
---
...
- The user asks for a frontline review, a shop-floor review, or an adoption-lens pressure-test of a document.
- The user is preparing a rollout, a process change or a tool deployment and wants to know where the people doing the work will push back or work around it.
- The user asks "what will the team actually say about this" or similar.
- The user is preparing for a staff briefing, an operations review or a works council conversation and wants the holes found first.

Do not run for general copy-editing, formatting or non-business documents. This skill reviews whether the change described will be worked with or worked around.
...
2. Optional context, ask only if not volunteered: which team the change lands on, the go-live date, and the decision being requested (approve, fund, proceed).
...
3. Check for /Documents/Cowork/org-profile.md in the user's OneDrive. If it exists, read it and use it as company context (industry, size, team structures, change history) throughout the review. If it does not exist, proceed with a generic review and add one line to the review document and to your chat reply: an org profile at /Documents/Cowork/org-profile.md would sharpen this review; the template ships with this skill at references/org-profile-template.md, so copy it, fill it in and save it as /Documents/Cowork/org-profile.md.
4. Read the artifact end to end before forming any finding. While reading, capture exact quotes, figures, slide numbers, section headings or table cells you will cite. Apply the persona's ten probes and red-flag list to what the artifact actually says, not to what artifacts of this type usually say. Where the artifact is silent on a probe (no training cover plan, no stop-doing list, no named team), record that silence as an open question; never invent a fact to fill the gap.
5. Compose the review with exactly these five sections, in this order:
   ...
   - TOP FINDINGS: three to seven findings, most important first. Every finding must quote or cite the specific part of the artifact it is about (a quoted sentence, a slide number, a named section, a table cell). Where the artifact is silent on something this persona probes, the finding states the silence and poses it as an open question. A finding that could be pasted under any rollout plan is not a finding; cut it.
   - RISKS: the material risks visible from the frontline lens (adoption collapse, workaround creation, training without cover, workload stacking with nothing removed, initiative fatigue, double entry from parallel running). Cite where in the artifact each risk arises, or state explicitly that the artifact is silent on it.
   - WHAT WOULD CHANGE MY MIND: the specific evidence, commitments or changes that would move the verdict up one level, phrased as the persona's explicit asks and drawn from its evidence standards.
   - 5 INTERROGATION QUESTIONS: exactly five questions this person would ask in the real meeting, in the persona's voice, ordered from the question most likely to open the meeting to the one most likely to close it. Draw on the persona's probes but anchor each question in this artifact's content.
6. Use the Word built-in skill to create the review as a .docx document. First line of the document: "DRAFT: Frontline Skeptic review of <artifact-name>, generated <date>". Then the five sections from step 5. Save it as <artifact-name>-frontline-skeptic-review.docx in /Documents/Cowork/reviews/. If the reviews folder does not exist, create it. If a file with that exact name already exists, do not overwrite it; save as <artifact-name>-frontline-skeptic-review-v2.docx (incrementing as needed) and tell the user why.
7. Verify the file exists at the named path before reporting completion. Then report back in chat: the verdict, the single most important finding, and the exact OneDrive path where the review document was saved. If the file landed somewhere other than /Documents/Cowork/reviews/ (for example a session folder), state the actual path and offer to copy the file to /Documents/Cowork/reviews/.

## OUTPUT ARTIFACTS

- /Documents/Cowork/reviews/<artifact-name>-frontline-skeptic-review.docx: the review document, labelled DRAFT on its first line, containing VERDICT, TOP FINDINGS, RISKS, WHAT WOULD CHANGE MY MIND and 5 INTERROGATION QUESTIONS.
...
- The persona is a role archetype. Never present its output as the opinion of any real, named individual or any real team, and never imply that actual frontline staff have seen or endorsed the review.
...
- [ ] The file was verified to exist at the named path and the exact path is reported in chat; any deviation from /Documents/Cowork/reviews/ is disclosed.
