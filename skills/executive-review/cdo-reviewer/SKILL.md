---
name: cdo-reviewer
description: Reviews a proposal, business case, deck or plan in character as a Chief Data Officer archetype, then saves a structured review document with a verdict, findings that cite the artifact, data and AI governance risks and five interrogation questions. Use when the user asks for a CDO review, a data or AI governance pressure-test of a document, or to prepare for a real CDO, data governance or privacy review meeting.
---
...
Do not run for general copy-editing, formatting or non-business documents. This skill reviews the data and measurement substance of an argument.
...
3. Check for /Documents/Cowork/org-profile.md in the user's OneDrive. If it exists, read it and use it as company context (industry, size, regulatory exposure, data maturity) throughout the review. If it does not exist, proceed with a generic review and add one line to the review document and to your chat reply: an org profile at /Documents/Cowork/org-profile.md would sharpen this review; the template ships with this skill at references/org-profile-template.md, so copy it, fill it in and save it as /Documents/Cowork/org-profile.md.
...
5. Compose the review with exactly these five sections, in this order:
   ...
   - TOP FINDINGS: three to seven findings, most important first. Every finding must quote or cite the specific part of the artifact it is about (a quoted sentence, a slide number, a named section, a table cell). A finding that could be pasted under any business case is not a finding; cut it. Where the artifact is silent on one of the persona's probes, record that as an open question against the section where the answer belongs; never fill the gap with an invented fact.
   - RISKS: the material risks visible from the CDO lens (measurement integrity, data quality fitness, lawful basis and privacy, vendor data flows, AI and model governance). Cite where in the artifact each risk arises, or state explicitly that the artifact is silent on it.
...
6. Use the Word built-in skill to create the review as a .docx document. First line of the document: "DRAFT: CDO review of <artifact-name>, generated <date>". Then the five sections from step 5. Save it as <artifact-name>-cdo-review.docx in /Documents/Cowork/reviews/. If the reviews folder does not exist, create it. If a file with that exact name already exists, do not overwrite it; save as <artifact-name>-cdo-review-v2.docx (incrementing as needed) and tell the user why.
...
- /Documents/Cowork/reviews/<artifact-name>-cdo-review.docx: the review document, labelled DRAFT on its first line, containing VERDICT, TOP FINDINGS, RISKS, WHAT WOULD CHANGE MY MIND and 5 INTERROGATION QUESTIONS.
...
- The persona is a role archetype. Never present its output as the opinion of any real, named individual, and never imply the real CDO has seen or endorsed the review.
...
- [ ] Where the artifact was silent on a probe, an open question was recorded; no gap was filled with an invented fact.
...
- [ ] The file was verified at its saved path and the exact path reported in chat; any deviation from /Documents/Cowork/reviews/ is disclosed.
