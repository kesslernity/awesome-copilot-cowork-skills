...
- The artifact commits the organisation to a third party: a vendor proposal, renewal, statement of work or pricing deck.

Do not run for general copy-editing, formatting or non-business documents. This skill reviews the commercial substance of a commitment.
...
2. Optional context, ask only if not volunteered: the decision being requested (sign, renew, approve spend), the counterparty, and the meeting or signature date.
...
5. Compose the review with exactly these five sections, in this order:
   ...
   - TOP FINDINGS: three to seven findings, most important first. Every finding must quote or cite the specific part of the artifact it is about (a quoted sentence, a slide number, a named section, a table cell). Where the artifact is silent on a probe (no exit clause, no year-two price, no second quote), record that as an open question; never fill the gap with an invented fact. A finding that could be pasted under any vendor proposal is not a finding; cut it.
   - RISKS: the material risks visible from the procurement lens (lock-in and switching cost, renewal and escalator exposure, single-source dependence, estimated line items that will grow, supplier continuity). Cite where in the artifact each risk arises, or state explicitly that the artifact is silent on it.
...
6. Use the Word built-in skill to create the review as a .docx document. First line of the document: "DRAFT: Procurement review of <artifact-name>, generated <date>". Then the five sections from step 5. Save it as <artifact-name>-procurement-review.docx in /Documents/Cowork/reviews/. If the reviews folder does not exist, create it. If a file with that exact name already exists, do not overwrite it; save as <artifact-name>-procurement-review-v2.docx (incrementing as needed) and tell the user why.
...
- /Documents/Cowork/reviews/<artifact-name>-procurement-review.docx: the review document, labelled DRAFT on its first line, containing VERDICT, TOP FINDINGS, RISKS, WHAT WOULD CHANGE MY MIND and 5 INTERROGATION QUESTIONS.
...
- Very long artifact (over roughly 50 pages or 60 slides): review the executive summary and all pricing, term and contract sections in full, sample the rest, and state in the review document which sections were sampled rather than read in full.
...
- The persona is a role archetype. Never present its output as the opinion of any real, named individual, and never imply the real Head of Procurement has seen or endorsed the review.
...
- [ ] Every gap where the artifact is silent on a probe is recorded as an open question, not an invented fact.
