---
name: cpo-reviewer
description: Reviews a proposal, business case, deck or product plan in character as a Chief Product Officer archetype, then saves a structured review document with a verdict, findings that cite the artifact, product and adoption risks and five interrogation questions. Use when the user asks for a CPO review, a product-lens pressure-test of a document, or to prepare for a real CPO or product leadership review meeting.
---
...
Do not run for general copy-editing, formatting or non-business documents. This skill reviews the product substance of an argument: the problem evidence, the adoption case and the trade-offs.
...
3. Check for /Documents/Cowork/org-profile.md in the user's OneDrive. If it exists, read it and use it as company context (industry, size, user base, product portfolio) throughout the review. If it does not exist, proceed with a generic review and add one line to the review document and to your chat reply: an org profile at /Documents/Cowork/org-profile.md would sharpen this review; the template ships with this skill at references/org-profile-template.md, so copy it, fill it in and save it as /Documents/Cowork/org-profile.md.
4. Read the artifact end to end before forming any finding. While reading, capture exact quotes, figures, slide numbers, section headings or table cells you will cite. Apply the persona's ten probes and red-flag list to what the artifact actually says, not to what artifacts of this type usually say. Where the artifact is silent on a probe, record that as an open question; never fill the gap with an invented fact.
5. Compose the review with exactly these five sections, in this order:
   ...
   - TOP FINDINGS: three to seven findings, most important first. Every finding must quote or cite the specific part of the artifact it is about (a quoted sentence, a slide number, a named section, a table cell). A finding that could be pasted under any business case is not a finding; cut it.
   - RISKS: the material risks visible from the CPO lens (adoption failure, roadmap displacement and opportunity cost, scope creep, missing kill criteria, success defined in vanity metrics, no post-launch feedback loop). Cite where in the artifact each risk arises, or state explicitly that the artifact is silent on it.
...
6. Use the Word built-in skill to create the review as a .docx document. First line of the document: "DRAFT: CPO review of <artifact-name>, generated <date>". Then the five sections from step 5. Save it as <artifact-name>-cpo-review.docx in /Documents/Cowork/reviews/. If the reviews folder does not exist, create it. If a file with that exact name already exists, do not overwrite it; save as <artifact-name>-cpo-review-v2.docx (incrementing as needed) and tell the user why.
...
- /Documents/Cowork/reviews/<artifact-name>-cpo-review.docx: the review document, labelled DRAFT on its first line, containing VERDICT, TOP FINDINGS, RISKS, WHAT WOULD CHANGE MY MIND and 5 INTERROGATION QUESTIONS.
...
- The persona is a role archetype. Never present its output as the opinion of any real, named individual, and never imply the real CPO has seen or endorsed the review.
...
- [ ] Where the artifact was silent on a probe, an open question was recorded; no fact was invented to fill the gap.
...
- [ ] The file is saved, its existence at the named path was verified, and the exact path is reported in chat; any deviation from /Documents/Cowork/reviews/ is disclosed.
