...
## How do I add my own persona to the review board?

A persona is one Markdown file. Copy [`review-board/references/persona-template.md`](skills/executive-review/review-board/references/persona-template.md), fill in the six sections (mandate, what I probe first, red flags, what convinces me, vocabulary and tone, known blind spots), save it as `references/persona-<role>.md` inside your installed copy of `review-board`, start a NEW conversation, and convene it by role name.

You may not need to write one: nine bench personas ship as standalone skills in the `executive-review` category, and seating one is a single file copy. The capacity maths (Cowork allows 20 companion files per skill; the installed board carries 12, so up to 8 bench personas fit, or all 9 if you delete the README from your installed copy) is in the [review-board README](skills/executive-review/review-board/README.md). Two rules carry over: archetypes only, never a real named individual; and fill in the blind spots honestly, because the red-team stage uses that section to challenge the persona's own findings.

## What is org-profile.md, and is my data safe in it?

`/Documents/Cowork/org-profile.md` is an optional context file for the review personas: business model, customers, stage, priorities, constraints, recent decisions. With it, persona findings land on your business instead of a generic one; without it, personas treat every organisation fact as unknown and turn context-dependent findings into open questions. To create it, copy [`review-board/references/org-profile-template.md`](skills/executive-review/review-board/references/org-profile-template.md), fill it in, and save it at that path (worked example in `example-org-profile.md`).

It stays in your OneDrive. Cowork reads and writes OneDrive and SharePoint files only, and no skill in this library instructs the model to send content to an external address or URL. Only two skills reach the open web at all: `news-monitor-digest` by design, and `red-team-validator` only when you explicitly authorise a web check; neither reads your org profile outward. The usual caveat applies: skill rules are instructions, not platform-enforced controls.
